...
export default function HeroSerpentine({ featured, locale }: Props) {
  return (
    <div style={{ position: 'relative', marginTop: 26, height: 360 }}>
      <svg style={{ position: 'absolute', top: 0, bottom: 0, insetInlineStart: '50%', width: 86, transform: 'translateX(-50%)', opacity: 0.92, pointerEvents: 'none' }} viewBox="0 0 90 360" fill="none" preserveAspectRatio="none">
      ...
      {ORBS.map((orb, idx) => {
        ...
        return (
          <Link key={dish.id} href={`/dish/${dish.id}` as '/'} style={{ textDecoration: 'none', ...posStyle }}>
            ...
            <span style={{ position: 'absolute', bottom: -6, insetInlineStart: '50%', transform: 'translateX(-50%)', background: 'var(--cream)', color: '#173324', fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: 11, padding: '4px 11px', borderRadius: 999, whiteSpace: 'nowrap', boxShadow: '0 6px 16px rgba(0,0,0,0.4)' }}>
              {nameShort}
            </span>
          </Link>
        )
