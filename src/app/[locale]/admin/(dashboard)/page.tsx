...
interface Counts { dishes: number; categories: number; branches: number; openBranches: number }

const stats = (c: Counts) => [
  { label: 'Total Dishes',  value: c.dishes,       href: '/admin/dishes',     icon: '🍽️' },
  { label: 'Categories',   value: c.categories,   href: '/admin/categories', icon: '📋' },
  { label: 'Branches',     value: c.branches,     href: '/admin/branches',   icon: '📍' },
  { label: 'Open Branches',value: c.openBranches, href: '/admin/branches',   icon: '🟢' },
]

export default function AdminDashboard() {
  const [counts, setCounts]   = useState<Counts>({ dishes: 0, categories: 0, branches: 0, openBranches: 0 })
  const [seeding, setSeeding] = useState(false)
  const [seedMsg, setSeedMsg] = useState('')

  useEffect(() => {
    fetch('/api/admin-counts').then(r => r.json()).then(j => {
