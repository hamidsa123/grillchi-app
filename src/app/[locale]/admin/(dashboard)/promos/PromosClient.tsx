...
import { savePromo, togglePromo } from '../../actions'
...
export default function PromosClient({ initialPromos }: { initialPromos: Promo[] }) {
  ...
  const saveEdit = async () => {
    ...
    const result = await savePromo(draft)
              ...
              {isEditing && draft ? (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 13, color: 'var(--cream-50)', marginTop: 4 }}>CTA: {p.cta_en}</div>
                    {dish && <div style={{ fontSize: 12, color: 'var(--cream-30)', marginTop: 6 }}>Linked: {dish.name_en}</div>}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 10, flexShrink: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => handleToggle(p)} style={{ ...ghostBtn, fontSize: 12, padding: '6px 14px' }}>
                        {p.active ? 'Hide' : 'Show'}
                      </button>
